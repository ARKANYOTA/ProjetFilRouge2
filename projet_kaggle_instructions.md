...
Deep learning models are widely used for image classification. The goal of this project is to compare state-of-the-art models on the TILDA textile texture dataset. The link of the project is : [https://www.kaggle.com/t/286b14f08b6941f3a1bb4b25e23f6f78](https://www.kaggle.com/t/286b14f08b6941f3a1bb4b25e23f6f78).
...
Refer to the website [https://pytorch.org/tutorials/beginner/basics/intro.html](https://pytorch.org/tutorials/beginner/basics/intro.html).
...
This means that $\mathbb{P}(S = 1 \mid y = k) = p_k$ and $\mathbb{P}(S = 0 \mid y = k) = 1 - p_k$. Then we choose $\epsilon$ according to $S$ as below :
- $\epsilon = 0$ if $S = 0$
...
Assume $\widehat{y}(X)$ is the prediction of a model computed from $X$. We shall separate the dataset set into 2 groups, one with $S = 0$, the other with $S = 1$. The bias of this model can be computed from a ratio of these two groups, using the $DI$ metric [4] (at the following link : [https://github.com/wikistat/Fair-ML-4-Ethical-AI/tree/master/Propublica](https://github.com/wikistat/Fair-ML-4-Ethical-AI/tree/master/Propublica)) defined as :

$$DI = \frac{\mathbb{P}(\widehat{y}(X) = 1 \mid S = 0)}{\mathbb{P}(\widehat{y}(X) = 1 \mid S = 1)}$$

In practice, a model is considered unbiased as long as the $DI$ metric is close to 1.

**Question 3.3 :** Study experimentally if $model_2$ is biased or not on test data. Compare your results to what you have with $model_1$.
...
[4] [https://hal.science/hal-03253111](https://hal.science/hal-03253111).

[5] [https://www.cnil.fr/fr/intelligence-artificielle/guide/conformite-des-systemes-dia-les-autres-guides-outils-et-bonnes-pratiques](https://www.cnil.fr/fr/intelligence-artificielle/guide/conformite-des-systemes-dia-les-autres-guides-outils-et-bonnes-pratiques).
