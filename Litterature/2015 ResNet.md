...
way networks have not demonstrated accuracy gains with                         general have small responses, suggesting that identity map-
extremely increased depth (e.g., over 100 layers).                             pings provide reasonable preconditioning.

3. Deep Residual Learning                                                      3.2. Identity Mapping by Shortcuts

3.1. Residual Learning                                                         A building block is shown in Fig. 2. Formally, in this paper we consider a building block defined as:
    Let us consider H(x) as an underlying mapping to be                        $$y = F(x, \{W_i\}) + x$$
fit by a few stacked layers (not necessarily the entire net),                  Here $x$ and $y$ are the input and output vectors of the layers considered. The function $F(x, \{W_i\})$ represents the
with x denoting the inputs to the first of these layers. If one                residual mapping to be learned. For the example in Fig. 2 that has two layers, $F = W_2 \sigma(W_1 x)$ in which $\sigma$ denotes
hypothesizes that multiple nonlinear layers can asymptoti-                     ReLU [29] and the biases are omitted for simplifying notations. The operation $F + x$ is performed by a shortcut connection and element-wise addition. We adopt the second nonlinearity after the addition (i.e., $\sigma(y)$, see Fig. 2).
cally approximate complicated functions2 , then it is equiv-                   The shortcut connections in Eqn.(1) introduce neither extra parameter nor computation complexity. This is not only
alent to hypothesize that they can asymptotically approxi-                     attractive in practice but also important in our comparisons between plain and residual networks. We can fairly
mate the residual functions, i.e., H(x) − x (assuming that                     compare plain/residual networks that simultaneously have the same number of parameters, depth, width, and computa-
the input and output are of the same dimensions). So                           tional cost (except for the negligible element-wise addition).
rather than expect stacked layers to approximate H(x), we                      The dimensions of $x$ and $F$ must be equal in Eqn.(1). If this is not the case (e.g., when changing the input/output
explicitly let these layers approximate a residual function                    channels), we can perform a linear projection $W_s$ by the shortcut connections to match the dimensions:
F(x) := H(x) − x. The original function thus becomes                                           $$y = F(x, \{W_i\}) + W_s x$$
F(x)+x. Although both forms should be able to asymptot-                    We can also use a square matrix $W_s$ in Eqn.(1). But we will show by experiments that the identity mapping is sufficient
ically approximate the desired functions (as hypothesized),                    for addressing the degradation problem and is economical, and thus $W_s$ is only used when matching dimensions.
the ease of learning might be different.                                       The form of the residual function $F$ is flexible. Experiments in this paper involve a function $F$ that has two or
    This reformulation is motivated by the counterintuitive                    three layers (Fig. 5), while more layers are possible. But if $F$ has only a single layer, Eqn.(1) is similar to a linear layer:
phenomena about the degradation problem (Fig. 1, left). As                     $y = W_1 x + x$, for which we have not observed advantages.
we discussed in the introduction, if the added layers can                      We also note that although the above notations are about fully-connected layers for simplicity, they are applicable to
be constructed as identity mappings, a deeper model should                     convolutional layers. The function $F(x, \{W_i\})$ can represent multiple convolutional layers. The element-wise
have training error no greater than its shallower counter-                     addition is performed on two feature maps, channel by channel.
part. The degradation problem suggests that the solvers                        3.3. Network Architectures
might have difficulties in approximating identity mappings
by multiple nonlinear layers. With the residual learning re-                      We have tested various plain/residual nets, and have ob-
formulation, if identity mappings are optimal, the solvers                     served consistent phenomena. To provide instances for dis-
may simply drive the weights of the multiple nonlinear lay-                    cussion, we describe two models for ImageNet as follows.
ers toward zero to approach identity mappings.                                 Plain Network. Our plain baselines (Fig. 3, middle) are
    In real cases, it is unlikely that identity mappings are op-               mainly inspired by the philosophy of VGG nets [41] (Fig. 3,
timal, but our reformulation may help to precondition the                      left). The convolutional layers mostly have 3×3 filters and
problem. If the optimal function is closer to an identity                      follow two simple design rules: (i) for the same output
mapping than to a zero mapping, it should be easier for the                    feature map size, the layers have the same number of fil-
solver to find the perturbations with reference to an identity                 ters; and (ii) if the feature map size is halved, the num-
mapping, than to learn the function as a new one. We show                      ber of filters is doubled so as to preserve the time com-
by experiments (Fig. 7) that the learned residual functions in                 plexity per layer. We perform downsampling directly by
                                                                               convolutional layers that have a stride of 2. The network
  2 This hypothesis, however, is still an open question. See [28].             ends with a global average pooling layer and a 1000-way
   ...
   std   2                                                                             ResNet-56
         ...
         0               20               40              60            80                   100
                                           layer index (original)
                                                                                       ...
                                                                                       ResNet-20
   std   2                                                                             ResNet-56                         ble 10 and 11 for better results.
                                                                                       ResNet-110
